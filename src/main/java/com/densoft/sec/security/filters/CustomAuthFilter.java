...
public class CustomAuthFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
        ...
        if (req.get("email") == null || req.get("password") == null) {
            SecurityUtil.sendResponse(response, "password and email required");
        }
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
        ...
        new ObjectMapper().writeValue(response.getOutputStream(), SecurityUtil.generateTokens((User) authResult.getPrincipal(), request.getServletPath()));
    }
